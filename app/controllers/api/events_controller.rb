class Api::EventsController < Api::BaseController
  ...
  def going
    ...
    respond_with @rvsp_going
  end
  
  def not_going
    @event = Event.find(params[:id])
    
    # Users not going to this event
  ...
  def posts
    ...
    respond_with @posts = Post.where(event_id: @event.id).order(created_at: :desc)
  end
  ...
  def rsvp_going
    ...
    if isNotGoing?(@event.id, userId)
      sql = "DELETE FROM event_members WHERE user_id = #{userId} AND event_id = #{@event.id}"
  ...
  protected
  def ev_params
    params.require(:event).permit(:event_name, :description, :community_id)
  end
end
